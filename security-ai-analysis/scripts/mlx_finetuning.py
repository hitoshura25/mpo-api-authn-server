...
{% endif %}"""
    ...
    def run_fine_tuning(self, training_data_dir: Path,
        ...
        Args:
            ...
            custom_output_name: Optional custom name for output model (legacy)
            custom_output_path: Optional direct path for structured output (preferred)
        ...
        # Use structured output path if provided, otherwise fall back to legacy
        if custom_output_path:
            output_model_path = custom_output_path
            logger.info(f"Using structured output path: {output_model_path}")
        else:
            output_model_path = self.config.get_output_model_path(custom_output_name)
            logger.info(f"Using legacy output path: {output_model_path}")
    ...
    def _save_training_metadata(self, output_path: Path, training_args: Dict[str, Any], duration: float):
        ...
        metadata = {
            ...
            "fine_tuning_config": asdict(self.config.fine_tuning)
        }
