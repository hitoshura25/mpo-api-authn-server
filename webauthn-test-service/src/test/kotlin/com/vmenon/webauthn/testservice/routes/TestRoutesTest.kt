...
class TestRoutesTest {
    ...
    private fun createTestApp(block: suspend ApplicationTestBuilder.() -> Unit) = testApplication {
        application {
    ...
    @Test
    fun `health endpoint returns healthy status`() = createTestApp {
    ...
    @Test
    fun `generate registration credential returns valid response`() = createTestApp {

        val challenge = ByteArray.fromBase64Url("dGVzdC1jaGFsbGVuZ2U").base64Url
        val requestBody = TestRegistrationRequest(
            challenge = challenge,
            rpId = "example.com",
        ...
        val response = client.post("/test/generate-registration-credential") {
    ...
    @Test
    fun `generate registration credential with invalid challenge returns error`() = createTestApp {
    ...
    @Test
    fun `generate authentication credential with valid keyPairId returns success`() = createTestApp {
        ...
        val registrationResponse = client.post("/test/generate-registration-credential") {
            contentType(ContentType.Application.Json)
        ...
        val authRequest = TestAuthenticationRequest(
            challenge = authChallenge,
            credentialId = registrationResult.credentialId!!,
            keyPairId = registrationResult.keyPairId
        )

        val authResponse = client.post("/test/generate-authentication-credential") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `generate authentication credential with invalid keyPairId returns error`() = createTestApp {
        val challenge = ByteArray.fromBase64Url("dGVzdC1jaGFsbGVuZ2U").base64Url
        val requestBody = TestAuthenticationRequest(
            challenge = challenge,
            credentialId = "test-credential-id",
            keyPairId = "invalid-keypair-id"
        )

        val response = client.post("/test/generate-authentication-credential") {
            contentType(ContentType.Application.Json)
            setBody(objectMapper.writeValueAsString(requestBody))
        }
    ...
    @Test
    fun `clear test data removes all key pairs`() = createTestApp {
    ...
    @Test
    fun `sessions endpoint returns active key pairs`() = createTestApp {
    ...
    @Test
    fun `malformed JSON request returns appropriate error`() = createTestApp {
        val response = client.post("/test/generate-registration-credential") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `missing required fields in request returns error`() = createTestApp {
        val response = client.post("/test/generate-registration-credential") {
            contentType(ContentType.Application.Json)
    ...
    @Test
    fun `authentication credential with invalid challenge format returns error`() = createTestApp {
