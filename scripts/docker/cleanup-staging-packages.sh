...
# Simplified to use standard Docker tags for all build types:
# - PR builds: pr-{pr-number} (e.g., pr-47)
# - Main builds: latest
# - Workflow dispatch: sha256-{short-sha}-branch-{branch-name}-{run-number}
# - This provides precise targeting for cleanup while maintaining performance.
#
# CLEANUP LOGIC:
...
6            if [[ "$CLEANUP_CONTEXT" == "workflow_dispatch" ]]; then
                ...
                log "🎯 Targeting: pr-*, latest, sha256-*-branch-* patterns (simplified)"
            fi
        fi
...
perform_staging_cleanup() {
    ...
    if [[ -n "$CLEANUP_PR_NUMBER" && "$CLEANUP_PR_NUMBER" != "MAIN_BRANCH" ]]; then
        ...
        log "📋 Package patterns:"
        log "  - Docker tags: pr-*, latest, sha256-*-branch-* (simplified)"
